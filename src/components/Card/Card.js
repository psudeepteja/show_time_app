...
export default function Card({ nowShowingData }) {
  ...
  const initialMoviesLength = 8;
  const [isOpen, setIsOpen] = useState(false);
  const [moviesLength, setMoviesLength] = useState(initialMoviesLength);
  ...
  const handleClick = (item) => {
    ...
    if (languageFormatGroups.length > 1) {
      ...
      router.push(
        `/movies/${city}/${label}?frmtid=${languageFormatGroups[0].fmtGrpId}&date=${languageFormatGroups[0].screenFormats[0].nextAvailableDate}`
      );
    }
  };
  ...
  const handleLangClick = () => {
    toggleModal();
    router.push(
      `/movies/${city}/${langData.label}?frmtid=${langInfo.fmtGrpId}&date=${langInfo.screenFormats[0].nextAvailableDate}`
    );
  };
    ...
    <>
          ...
          {langData.languageFormatGroups.map((i, idx) => (
            <div key={idx} className="flex">
              <input
                ...
                name="lang"
                value={i.lang}
                onChange={() => handleLangChange(i)}
                checked={langInfo.lang === i.lang}
              />
              <p>{i.lang}</p>
            </div>
          ))}
          <p
            className="bg-orange-100 px-4 py-2 text-white font-semibold text-center"
      ...
      <div className="flex justify-center mt-6">
        {moviesData.length < nowShowingData?.groupedMovies.length && (
          <div
            className="px-6 py-2 border rounded-xl shadow-lg text-orange-100 font-semibold cursor-pointer text-xs"
